...
class doLDA:
    ...
    def fit_model(self, num_topics, alp, bt, rndst, iters):
        ...
        lda_model = gs.models.LdaMulticore(corpus=self.bow_corpus, num_topics=num_topics, id2word=self.dictionary,
                                               random_state=rndst, chunksize=chnksz, passes=pss, alpha=alp, eta=bt, iterations = iters)
#         For Topic printing
...
def fitdocs(inst, flda, dict_of_corps, agg = False):
    doc_topics = {'{}'.format(p):Counter([(sorted(flda.get_document_topics(inst.dictionary.doc2bow(d)), key = operator.itemgetter(1))[-1][0]+1) 
                                          for d in c]) 
                  for p, c in dict_of_corps.items()}
    tops = [d for c in dict_of_corps.values() for d in c]
    doc_topics['Total'] = Counter([(sorted(flda.get_document_topics(inst.dictionary.doc2bow(d)), key = operator.itemgetter(1))[-1][0]+1) 
                                              for d in tops]) 
