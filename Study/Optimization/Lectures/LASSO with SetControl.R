...
lasso.fun <- function(y.vec, x.mat, b.vec, lam, iter.max = 1e3, eps = 1e-8) {
  for (iter in 1:iter.max) {
...
lasso.path.fun <- function(y.vec, x.mat, lam.vec, iter.max = 1e3, eps = 1e-5) { # LASSO with set control
  ...
  for (lam in lam.vec) {
    for (iter in 1:iter.max) {
      ...
      fit <- lasso.fun(y.vec, ax.mat, b.vec[v.vec], lam)
...
lasso.path.fun(y.vec, x.mat, lam.vec = .1, iter.max = 1e3, eps = 1e-5)